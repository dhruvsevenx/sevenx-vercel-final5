"""Vercel Python serverless entry point — FastAPI backend.

This file is auto-detected by Vercel (any `.py` in the top-level `/api/`
folder becomes a Serverless Function). Vercel's Python runtime detects the
ASGI `app` variable and wraps it automatically.

MongoDB is OPTIONAL:
  * If `MONGO_URL` is set in Vercel env vars → Mongo endpoints work.
